...
if(cart.length > 0) {
    cart.forEach(product => {
        ...
        addToCartBtnsDOM.forEach(addToCartBtnDOM => {
            ...
            if(productDOM.querySelector('.product__name').textContent === product.name){
...
function handleActionsOnBtns(addToCartBtnDOM, product){
    ...
    cartItemsDOM.forEach(cartItemDOM => {
        if(cartItemDOM.querySelector('.cart__item__name').textContent === product.name){
            cartItemDOM.querySelector('[data-action="INCREASE_ITEM"]').addEventListener('click', _ => increaseItem(product, cartItemDOM));

            cartItemDOM.querySelector('[data-action="DECREASE_ITEM"]').addEventListener('click', _ => decreaseItem(product,cartItemDOM, addToCartBtnDOM));

            cartItemDOM.querySelector('[data-action="REMOVE_ITEM"]').addEventListener('click', _ => removeItem(product,cartItemDOM, addToCartBtnDOM));
        }
    });
}
...
function increaseItem(product, cartItemDOM) {
    cart.forEach(cartItem => {
        if(cartItem.name === product.name){
            cartItemDOM.querySelector('.cart__item__quantity').textContent = ++cartItem.quantity;
...
function addCartFooter(){
    if(!document.querySelector('.cart-footer')){
        ...
        `);
        document.querySelector('[data-action="CLEAR_CART"]').addEventListener('click', _=>clearItems());
        document.querySelector('[data-action="CHECKOUT"]').addEventListener('click', _=>checkout());
    }
}
// clear all items
function clearItems(){
...
function countCartTotal(){
    ...
    document.querySelector('[data-action="CHECKOUT"]').textContent = `Pay $${initialCount}`;
}
